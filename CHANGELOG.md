# 更新日志

所有显著变更记录于此。格式基于 [Keep a Changelog](https://keepachangelog.com/zh-CN/1.1.0/),
版本号遵循语义化版本;每条 Release 的详细说明由 `scripts/release-notes.mjs`
从 Conventional Commits 自动生成(见 AGENTS.md 的发版规范)。

## [1.3.0] - 2026-09-17

### 新功能
- 我的缓存多库管理:侧栏 ＋ 新建/导入/同步,库可切换、设封面、备份、删除,
  库内歌单分组折叠展示;「同步汽水音乐缓存为库」一键建库(填名+图标可选),
  移除自动建档 —— 库完全由用户主动操作产生(Issue #32 #35 #37 #41 #43 #63 #67)
- 歌单包导出/导入 ZIP:选歌单选歌曲(勾选/全选/搜索/计数),库名+图标,
  导出 `<库名>-qsyy.zip`,导入保留 名称/图标/歌单分组/歌曲顺序/封面(Issue #41)
- 歌单排序:拖拽自定义排序 + 字母/时长/随机/歌词分组,「同步收藏」重置为
  官方顺序(Issue #39)
- 未登录也可用:库内已缓存歌曲走库限定流 `/api/stream/<id>?set=<库>` 直取
  本地音频,不做客户端扫描/在线解析;启动不再被歌单加载失败阻塞,
  非写入库可直接点播 —— 含实际音频的导入库无需登录即可完整使用(Issue #69)

### 修复
- 行尾缓存环可点击,提供「清理缓存 / 重加载缓存」;列表重渲染/切换歌单后
  缓存状态瞬时回填,不再全空(Issue #45 #47)
- 库行加删除按钮;活动库可删(二次确认),「使用中」拆分为「播放中/写入库」,
  播放行高亮按来源上下文门控不再跨库串染(Issue #49 #53 #55)
- ZIP 导入补齐歌手/专辑/时长元数据;导入包自动按 qsyy.json 命名建库,
  合并/替换在选完文件后的弹窗选择(Issue #37 #43 #69)

[1.3.0]: https://github.com/shiaho777/qsyy/releases/tag/v1.3.0

## [1.2.0] - 2026-09-07

### 新功能
- 封面主题色:整窗背景随当前播放封面主色实时渐变(左下深→右上浅对角,
  切歌平滑过渡),取色对齐官方客户端封面五基色思路,本地 canvas 采样实现(Issue #30)
- 界面品质整体升级:墨色渐变设计系统、玻璃拟态侧栏与播放条、白色实心播放键、
  封面加载淡入、面板弹性入场、首帧淡入、toast 出场动画(Issue #30)
- 点击播放栏封面查看大图(Esc 或点击空白关闭);侧栏新增品牌区,
  顶部留白避让 macOS 红绿灯并兼作窗口拖拽区(Issue #30)

### 修复
- 歌词 / 播放队列 / 下载任务按钮支持再次点击关闭(toggle),
  按钮带打开态高亮;修复面板关闭动画期间重复点击被误关的问题(Issue #30)
- 主进程退出时统一收掉全部子进程(此前仅 ttnet-helper):扫描 / 解密 / 下载
  子进程统一注册并按进程组整树收尸,不再残留孤儿进程;
  restore_cache.js 的 runScanChild 显式传递 ELECTRON_RUN_AS_NODE(Issue #30)
- 桌面开发壳 Dock 图标与应用名显示为 Electron 默认值(Issue #30)

[1.2.0]: https://github.com/shiaho777/qsyy/releases/tag/v1.2.0

## [1.1.0] - 2026-09-03

### 新功能
- 三端应用图标统一为汽水罐:SVG 矢量源 + `scripts/make-icons.py` 一键生成
  (桌面 icns/ico、Android 自适应图标、PWA manifest 与 favicon)，替换旧占位
  SVG(Issue #24)
- 侧栏底部 GitHub 行:仓库入口 + 版本号展示，右侧内嵌「检查更新」按钮，
  对比 GitHub 最新 Release，有新版直达对应 release 页(Issue #23)
- 新增 `GET /api/version`(版本号 + 仓库地址)

[1.1.0]: https://github.com/shiaho777/qsyy/releases/tag/v1.1.0

## [1.1.1] - 2026-09-03

### 重构
- 侧栏底部「管理缓存」不再独占一行，与「同步收藏」并排(Issue #26)

[1.1.1]: https://github.com/shiaho777/qsyy/releases/tag/v1.1.1

## [1.1.2] - 2026-09-03

### 修复
- 检查更新撞上 GitHub API 限流时误报连不上:新增 `GET /api/latest-release`
  服务端兜底（读网页 302 跳转拿 tag），失败时直接打开 Releases 页(Issue #28)

[1.1.2]: https://github.com/shiaho777/qsyy/releases/tag/v1.1.2

## [1.0.2] - 2026-09-01

### 修复
- 桌面版本地缓存全部显示为未缓存:Electron 内 spawn 的 Node 子进程(scan /
  decrypt / ttnet-helper / restore)缺 `ELECTRON_RUN_AS_NODE=1`,被应用二进制
  静默吞掉,缓存扫描永远为空(Issue #20)
- 顺带确认 v1.0.1 的单实例锁正常,连续点击 Dock 图标不再叠开窗口

[1.0.2]: https://github.com/shiaho777/qsyy/releases/tag/v1.0.2

## [1.0.1] - 2026-09-01

### 修复
- 桌面壳加单实例锁:重复启动(Dock 图标反复点击 / 安装时旧实例存活)不再叠开
  多个窗口,第二次启动会把已有窗口带回前台(Issue #16)
- 启动探针改用瞬时端点并加 1.5s 超时,首次启动不再长时间白屏
  (原探针会等待签名子进程最长 4 秒)

[1.0.1]: https://github.com/shiaho777/qsyy/releases/tag/v1.0.1

## [1.0.0] - 2026-09-01

### 新功能
- 三端客户端:macOS(dmg/zip)、Windows(nsis 安装器/zip)、Android(apk),
  tag 推送后由 GitHub Actions 自动构建并发布到 Release
- 更新日志自动化:release workflow 按 Conventional Commits 生成发布说明
- `desktop/`:Electron 壳,进程内加载 standalone 服务,零逻辑重复
- `android/`:WebView 壳,首次启动配置服务端地址,前台服务保活播放
- `.github/workflows/release.yml`:三端矩阵构建 + Release 发布
- `.github/workflows/ci.yml`:push/PR 门禁(语法检查、workflow 校验、文档链接)
- `AGENTS.md`:交付 / 发版行为规范与项目经验库

### 文档
- README 重写为"核心功能"驱动结构(音效 / 音质 / 源文件与转码 / 三层缓存机制)
- PLATFORM-macOS.md 补全环境变量总表、HTTP API 一览、播放通路降级链
- 平台范围收敛:服务端 macOS / Windows,移动端 Android

[1.0.0]: https://github.com/shiaho777/qsyy/releases/tag/v1.0.0
